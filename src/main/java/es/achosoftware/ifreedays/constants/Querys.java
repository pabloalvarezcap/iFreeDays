...
public class Querys {
	...
	public static final String FIND_USERS_BY_PROJECT_ID = "select * from user u" + " inner join user_projects up on (up.user_id = u.user_id) " + " where up.project_id = :projectId";

	public static final String FIND_SKILLS_BY_PROJECT_ID = "select * from skill s" + " inner join user_skills us on (us.user_id = s.user_id) " + " inner join user_projects up on (up.user_id = us.user_id) " +  " where up.project_id = :projectId";

	private static final String VACATIONS_FOR_SKILL_AND_FOR_PROJECT = "select v.* from vacation v"
			+ " inner join user_skill us on (us.user_id = v.user_id)" + " inner join user_projects up on (up.user_id = us.user_id)" + " where us.skill_id = :skillId AND up.projec_id = :projectId";
	...
	public static final String FIND_VACATIONS_BY_USER_ID_AND_BY_PROJECT_ID = "select * from vacation v" + "inner join user_projects up on (up.user_id = v.user_id)" + "where v.user_id = :userId AND up.projec_id = :projectId";
}
